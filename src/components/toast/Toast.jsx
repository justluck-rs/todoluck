 ...
 * @param {number} props.duration - Duração em ms (padrão: 3000ms)
...
export function Toast({ message, type = 'info', duration = 3000, onClose }) {
  ...
  const getClasses = () => {
    ...
    switch (type) {
      case 'success':
        return `${baseClasses} bg-green-500/20 text-white border-green-500/30`;
      case 'error':
        return `${baseClasses} bg-red-500/20 text-white border-red-500/30`;
      case 'warning':
        return `${baseClasses} bg-yellow-500/20 text-white border-yellow-500/30`;
      default: // info
        return `${baseClasses} bg-blue-500/20 text-white border-blue-500/30`;
    }
  };
